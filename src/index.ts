...
class JsonQueryMCP {
  ...
  private async getDefaultFileSchema(): Promise<any> {
            ...
            inputSchema: {
              ...
              properties: {
                filePath: {
                  type: "string",
                  description:
                    "Path to the JSON file to query (can be absolute or relative). If not provided, uses the default path configured for the server.",
                },
                query: {
                  type: "string",
                  description:
                    'JSONPath expression (e.g., "$.store.book[*].title", "$.organization.divisions.*.manager.name")',
                },
              },
              required: [],
            },
            ...
            inputSchema: {
              ...
              properties: {
                filePath: {
                  type: "string",
                  description:
                    "Path to the JSON file to generate schema for (can be absolute or relative). If not provided, uses the default path configured for the server.",
                },
              },
              required: [],
            },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      if (name === "query-json") {
        ...
        if (!actualFilePath) {
          throw new McpError(
            ErrorCode.InvalidRequest,
            "No file path provided and no default path configured"
          )
        }
  ...
  async run(): Promise<void> {
    ...
    console.error(
      `JSON Query MCP Server running on stdio (PID: ${process.pid})`
    )
  }
}
